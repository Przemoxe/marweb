...
class Breadcrumb
{
    ...
    public function __construct( $args = array() )
    {
        ...
        $this->args = apply_filters( 'breadcrumb_args', wp_parse_args( $args, $defaults ) );
        ...
        if(isset($this->args['is_archival'])){
            $archivePage = get_page_by_path( 'archiwum-produktow' );
            $translatedArchivePage = get_page(icl_object_id($archivePage->ID));
            $this->items = [];
            $this->add_network_home_link();
            $this->add_site_home_link();
            $this->items[] = "<a href='".get_permalink($translatedArchivePage)."'>".$translatedArchivePage->post_title."</a>";
            if (is_singular())
            {
    ...
    public function trail()
    {
        ...
        $breadcrumb    = '<div class="breadcrumb-wrap-content">';
        ...
        if (0 < $item_count)
        {
            ...
            $breadcrumb = sprintf(
                '<div class="breadcrumb-wrap"><%1$s role="navigation" aria-label="%2$s" class="breadcrumb-trail breadcrumbs" itemprop="breadcrumb">%3$s%4$s%5$s</%1$s></div>',
        ...
        $breadcrumb = apply_filters( 'breadcrumb', $breadcrumb, $this->args );
    ...
    protected function set_labels()
    {
        $defaults = array(
            ...
            'home'                => esc_html__( 'Strona główna', TEXT_DOMAIN ),
            'error_404'           => esc_html__( '404', TEXT_DOMAIN ),
            'archives'            => esc_html__( 'Archiwum', TEXT_DOMAIN ),
            // Translators: %s is the search query. The HTML entities are opening and closing curly quotes.
            'search'              => esc_html__( 'Wyniki wyszukiwania dla &#8220;%s&#8221;', TEXT_DOMAIN ),
            // Translators: %s is the page number.
            'paged'               => esc_html__( 'Strona %s', TEXT_DOMAIN ),
            // Translators: Minute archive title. %s is the minute time format.
            'archive_minute'      => esc_html__( 'Minuta %s', TEXT_DOMAIN ),
            // Translators: Weekly archive title. %s is the week date format.
            'archive_week'        => esc_html__( 'Tydzień %s', TEXT_DOMAIN ),
        ...
        $this->labels = apply_filters( 'breadcrumb_labels', wp_parse_args( $this->args['labels'], $defaults ) );
    }
    ...
    protected function set_post_taxonomy()
    {
        ...
        $this->post_taxonomy = apply_filters( 'breadcrumb_post_taxonomy', wp_parse_args( $this->args['post_taxonomy'], $defaults ) );
    }
    ...
    protected function add_items()
    {
        ...
        $this->items = array_unique( apply_filters( 'breadcrumb_items', $this->items, $this->args ) );
    }
