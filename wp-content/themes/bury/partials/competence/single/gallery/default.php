        ...
        <div class="product-gallery__container">
            ...
            <?php foreach($items as $index => $item): ?>
                ...
                <?php if ($item->thumbnail_type == 'image'): ?>
                    <div data-item-index="<?= $index ?>" class="product-gallery__list-item product-gallery__list-item--type-image <?= array_get($sizeClassOptions, $item->size) ?> <?= $current ?> " style="background-image: url('<?= wp_get_attachment_image_url($item->featured_image, 'full'); ?>');" anchor-link>
                ...
                <?php else: ?>
                    <div data-item-index="<?= $index ?>" class="product-gallery__list-item product-gallery__list-item--type-background <?= array_get($sizeClassOptions, $item->size) ?> <?= $current ?> " style="background-color:  <?= $bgColor ?>" anchor-link>
                        <div class="product-gallery__list-item--content">
            ...
            <?php foreach($items as $index => $item): ?>
                ...
                <!-- rotator -->
                <div data-item-index="<?= $index ?>" id="product-gallery-slider-<?= $index ?>" class="<?= $active ?> product-gallery__item product-gallery__slider br-block carousel slide carousel-fade" data-ride="carousel" data-interval="4000">
                    <!-- Slides -->
                ...
                <!-- title -->
                <?php if($index === 0) : ?> <p class="anchor" anchor></p> <?php endif;?>
                <div data-item-index="<?= $index ?>" class="product-gallery__item product-gallery__title-content <?= $active ?>" >
                    <div class="product-gallery__title-box">
                        <div>
                            <h2 class="product-gallery__item product-gallery__title"><?= $item->title ?></h2>
